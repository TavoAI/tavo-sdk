...
namespace TavoAI
{
    ...
    public class PluginMarketplaceClient
    {
        ...
        public async Task<Task<PluginInstallationResponse>> post{plugin_id}install(string plugin_id, string? organization_id = null)
        ...
        public async Task<Task<object>> get{plugin_id}download(string plugin_id, string? version = null)
        {'                    var queryParams = new Dictionary<string, object?> { "plugin_id", plugin_id, "version", version };\n                    var queryString = string.Join("&", queryParams.Where(p => p.Value != null).Select(p => $"{p.Key}={Uri.EscapeDataString(p.Value.ToString())}"));\n                    var url = "/{plugin_id}/download" + (string.IsNullOrEmpty(queryString) ? "" : $"?{queryString}");\n                    var content = null;\n                    var response = await this.httpClient.GetAsync(url);\n                    response.EnsureSuccessStatusCode();\n                    return await response.Content.ReadFromJsonAsync<object>();'}

        /// <summary>
        /// GET /installed
        /// </summary>
        
        public async Task<Task<List[PluginInstallationResponse]>> getinstalled()
        {'                    var url = "/installed";\n                    var content = null;\n                    var response = await this.httpClient.GetAsync(url);\n                    response.EnsureSuccessStatusCode();\n                    return await response.Content.ReadFromJsonAsync<object>();'}
        ...
        public async Task<Task<PluginResponse>> put{plugin_id}(string plugin_id, object plugin_data)
        ...
        /// </summary>
                /// <param name="plugin_id">plugin_id parameter</param>
        public async Task<Task<object>> delete{plugin_id}(string plugin_id)
        ...
        /// </summary>
                /// <param name="plugin_id">plugin_id parameter</param>
        public async Task<Task<PluginResponse>> post{plugin_id}publish(string plugin_id)
        ...
        public async Task<Task<PluginVersionResponse>> post{plugin_id}versions(string plugin_id, object version_data)
        ...
        /// </summary>
                /// <param name="plugin_id">plugin_id parameter</param>
        public async Task<Task<List[PluginVersionResponse]>> get{plugin_id}versions(string plugin_id)
        {'                    var queryParams = new Dictionary<string, object?> { "plugin_id", plugin_id };\n                    var queryString = string.Join("&", queryParams.Where(p => p.Value != null).Select(p => $"{p.Key}={Uri.EscapeDataString(p.Value.ToString())}"));\n                    var url = "/{plugin_id}/versions" + (string.IsNullOrEmpty(queryString) ? "" : $"?{queryString}");\n                    var content = null;\n                    var response = await this.httpClient.GetAsync(url);\n                    response.EnsureSuccessStatusCode();\n                    return await response.Content.ReadFromJsonAsync<object>();'}
        ...
        /// <param name="sort_by">sort_by parameter</param>
        /// <param name="sort_order">sort_order parameter</param>
        public async Task<Task<List[PluginReviewResponse]>> get{plugin_id}reviews(string plugin_id, double? page = null, double? limit = null, double? min_rating = null, string? sort_by = null, string? sort_order = null)
        {'                    var queryParams = new Dictionary<string, object?> { "plugin_id", plugin_id, "page", page, "limit", limit, "min_rating", min_rating, "sort_by", sort_by, "sort_order", sort_order };\n                    var queryString = string.Join("&", queryParams.Where(p => p.Value != null).Select(p => $"{p.Key}={Uri.EscapeDataString(p.Value.ToString())}"));\n                    var url = "/{plugin_id}/reviews" + (string.IsNullOrEmpty(queryString) ? "" : $"?{queryString}");\n                    var content = null;\n                    var response = await this.httpClient.GetAsync(url);\n                    response.EnsureSuccessStatusCode();\n                    return await response.Content.ReadFromJsonAsync<object>();'}
        ...
        public async Task<Task<PluginReviewResponse>> post{plugin_id}reviews(string plugin_id, object review_data)
        ...
        public async Task<Task<PluginReviewResponse>> get{plugin_id}reviews{review_id}(string plugin_id, string review_id)
        {'                    var queryParams = new Dictionary<string, object?> { "plugin_id", plugin_id, "review_id", review_id };\n                    var queryString = string.Join("&", queryParams.Where(p => p.Value != null).Select(p => $"{p.Key}={Uri.EscapeDataString(p.Value.ToString())}"));\n                    var url = "/{plugin_id}/reviews/{review_id}" + (string.IsNullOrEmpty(queryString) ? "" : $"?{queryString}");\n                    var content = null;\n                    var response = await this.httpClient.GetAsync(url);\n                    response.EnsureSuccessStatusCode();\n                    return await response.Content.ReadFromJsonAsync<object>();'}
        ...
        public async Task<Task<PluginReviewResponse>> put{plugin_id}reviews{review_id}(string plugin_id, string review_id, object review_update)
        ...
        /// </summary>
                /// <param name="plugin_id">plugin_id parameter</param>
        /// <param name="review_id">review_id parameter</param>
        public async Task<Task<object>> delete{plugin_id}reviews{review_id}(string plugin_id, string review_id)
        ...
        /// </summary>
                /// <param name="plugin_id">plugin_id parameter</param>
        /// <param name="review_id">review_id parameter</param>
        public async Task<Task<object>> post{plugin_id}reviews{review_id}helpful(string plugin_id, string review_id)
